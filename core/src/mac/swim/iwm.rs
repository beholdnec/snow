...
impl Swim {
    ...
    fn iwm_tick_bitstream(&mut self, ticks: Ticks) -> Result<()> {
        self.bitstream_cycles += ticks;
        if self.bitstream_cycles < self.get_selected_drive().get_ticks_per_bit() {
            return Ok(());
        }

        self.bitstream_cycles -= self.get_selected_drive().get_ticks_per_bit();
