    ...
    TBus: Bus<Address, u8> + IrqSource,
{
    ...
    /// Cycle counter of last bus sync
    pub bus_sync_cycles: Ticks,
    ...
    TBus: Bus<Address, u8> + IrqSource,
{
    ...
    fn sync_bus(&mut self) -> Result<()> {
        let cycles_since_last_sync = self.cycles - self.bus_sync_cycles;
        if cycles_since_last_sync > 0 {
            self.bus_sync_cycles += cycles_since_last_sync;
